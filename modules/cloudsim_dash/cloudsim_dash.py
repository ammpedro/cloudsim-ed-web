    ...
    def post(self):
        ...
        if action == "checkstatus":
            try:
                ...
                SessionHandler.session['ip_addr'] = ip_address
                SessionHandler.session['usrname'] = username
                SessionHandler.session['passwd'] = password
                SessionHandler.session['sim_name'] = simulator_name
            ...
            try:
                ip_address = SessionHandler.session.get('ip_addr')
                username = SessionHandler.session.get('usrname')
                password = SessionHandler.session.get('passwd')
                simulator_name = SessionHandler.session.get('sim_name')
                ...
                cloudsim_api = CloudSimRestApi(ip_address, username, password)
                task_id = cloudsim_api.create_task(simulation_name, task_dict)
                tasks = cloudsim_api.get_tasks(simulation_name)
                status = str(tasks)
